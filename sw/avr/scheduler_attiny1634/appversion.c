/**
 * @addtogroup SCHEDULER_ATTINY1634
 * @addtogroup SCHEDULER_ATTINY1634_APPVERSION
 * @brief Version information of "testscheduler" project.
 *
 * @{
 * @file    appversion.c
 * @brief   Version information of "testscheduler" project.
...
const unsigned char app_versioninfo[MOD_LEN_CONTROLLERID + MOD_LEN_BOARDID + MOD_LEN_BOARDREV + MOD_LEN_APPID + MOD_LEN_APPVER] __attribute__((section(".versioninfo")))
                        = {SIGNATURE_0,SIGNATURE_1,SIGNATURE_2,0x00,
                           0x00,0x03,   // board ID            (high byte, low byte)
                           1,           // board revision
                           0x00,0x01,   // application ID      (high byte, low byte)
                           0,1,2};      // application version (major, minor, bugfix)
