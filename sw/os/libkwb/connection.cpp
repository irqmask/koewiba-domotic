...
{
    if (this->extOnIncomingMsg != nullptr) {
        this->extOnIncomingMsg(message, this);
    }
}
...
//----------------------------------------------------------------------------
bool Connection::addressIsInConnectionsSegment(uint16_t nodeAddress)
{
    if ((segmentId == 0) ||
        (nodeAddress == 0) ||
        ((nodeAddress & BUS_SEGBRDCSTMASK) == segmentId)) {
        return true;
    }
    return false;
}
