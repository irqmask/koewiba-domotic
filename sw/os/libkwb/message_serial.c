...
static void reset_incoming_on_error(msg_serial_t *msg_serial)
{
    ...
    for (ii = 0; ii < msg_serial->incoming_num_received; ii++) {
        ...
        case '\n':
            strcat_s(strbuf1, sizeof(strbuf1), "<NL>");
            break;
        case '\r':
            strcat_s(strbuf1, sizeof(strbuf1), "<CR>");
            break;
        default:
    ...
    snprintf(strbuf2, sizeof(strbuf2) - 1, "\" total length = %d, message length = %d\n", msg_serial->incoming_num_received,
             msg_serial->incoming_message.length);
...
static bool process_receiving(msg_serial_t *msg_serial, char new_char)
{
    ...
    case eSER_RECV_STATE_DATA:
        ...
        if (convert_char_to_nibble(new_char, &nibble) != 0) {
            ...
            if (digit == 1) {
                msg_serial->incoming_message.data[index] = 0;
            }
    ...
    case eSER_RECV_STATE_NEWLINE:
        if (msg_serial->incoming_num_received != (msg_serial->incoming_message.length * 2 + 11) ||
            ...
            // message received
            if (msg_serial->incoming_num_received) {
                msg_serial->incoming_num_received--;
            }
...
static int32_t msg_read(void *arg)
{
    ...
    do {
        ...
        do {
            ...
            if (1 != sys_serial_recv(msg_serial->fd, &new_char, 1)) {
                break;
            }
...
static size_t format_serial_message(char *serial_buffer, uint8_t max_buffersize, msg_t *message)
{
    ...
    if (max_buffersize < (message->length * 2 + 7)) {
        return 0;
    }
